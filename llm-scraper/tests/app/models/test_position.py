...
def test_init_position(build_db):
    attrs = {'job_id': 'bd0f06b3dbd568d7', 'job_title': 'Data Engineer', 'company_name': 'Unity Catalog - Data Bricks', 'seniority_level': None,
         'min_salary': None, 'max_salary': None, 'city': 'unknown', 
         'state': None, 'zipcode': None, 'presence': 'remote', 'scraping_id': 1}
    position = Position(**attrs)
    assert position.job_id == 'bd0f06b3dbd568d7'
    assert position.job_title == 'Data Engineer'

def test_position_has_scraper(build_db):
    scraping_attrs = {'date': datetime.date(2024, 3, 1), 'id': 1, 'position': 'data_engineer', 'location': 'united_states', 'job_idx': 2}
    scraping = Scraping(**scraping_attrs)
    attrs = {'job_id': 'bd0f06b3dbd568d7', 'job_title': 'Data Engineer', 'company_name': 'Unity Catalog - Data Bricks', 'seniority_level': None}
    position = Position(**attrs)
    position.scraping = scraping
    assert position in scraping.positions
